...
class App {
  ...
  constructor() {
    ...
    this.controls = new Controls(controlsContainer, this.settings, {
      ...
      onExampleLoad: (filename, label, presetUrl) =>
        this.handleExampleLoad(filename, label, presetUrl),
  ...
  private async handleExampleLoad(
    filename: string,
    _label: string,
    presetUrl?: string
  ): Promise<void> {
  ...
  private loadInitialAsset(): void {
    ...
    if (saved) {
      ...
      this.handleExampleLoad(
        '/image_assets/needle-and-thread.svg',
        'Needle and Thread',
        '/presets/needle-and-thread.json'
      )
    }
  }
}
